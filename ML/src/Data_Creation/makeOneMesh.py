...
# --- Configuration ---
model_folder = r"C:\Dokumente\Studium\6. Semester\Bachelorarbeit\Code\models_smplx_v1_1\models"  # Update as needed.
output_folder = r"C:\Dokumente\Studium\6. Semester\Bachelorarbeit\Code\smplx_output"
...
# Joint 16 is the left shoulder: rotate by -60° about the z-axis.
body_pose[:, 15*3:15*3+3] = torch.tensor([[0, 0, -np.deg2rad(60)]], dtype=torch.float32)
# Joint 17 is the right shoulder: rotate by +60° about the z-axis.
body_pose[:, 16*3:16*3+3] = torch.tensor([[0, 0, np.deg2rad(60)]], dtype=torch.float32)

# Slightly vary the body_pose using Gaussian noise.
pose_noise_std = 0.05  # Standard deviation in radians; adjust as needed.
noisy_body_pose = body_pose + torch.randn_like(body_pose) * pose_noise_std

# Set the global orientation to place the body lying down.
global_orient = torch.tensor([[np.pi/2, 0, 0]], dtype=torch.float32)

# --- Forward pass through the model ---
output = model.forward(betas=betas, body_pose=noisy_body_pose, global_orient=global_orient)
